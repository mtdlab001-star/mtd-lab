...
export default async function AgentsPage(){
  ...
  return <div className="shell"><aside className="side"><div className="brand">MTD Lab</div><div className="nav"><Link href="/">Dashboard</Link><Link href="/taxpayers">Taxpayers</Link><Link href="/agents">Agents</Link><Link href="/taxpayers/sandbox">Sandbox setup</Link></div><div className="operator">Operated by Glomaxel IT Service</div></aside><main className="main">
    ...
    <section className="panel" style={{marginTop:24}}><div className="sectionHead"><div><h2>Agent register</h2><p className="muted">An agent must be active and separately authorised for each taxpayer and permitted action. Expired or revoked relationships are excluded from active counts.</p></div><strong>{rows.filter((a:any)=>a.status==='active').length} active agents</strong></div>
