...
import { getClientCapacity } from '@/lib/subscription-capacity'
...
export default async function SandboxTaxpayerPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  const qs=await searchParams
  const workspace=await currentWorkspace()
  if(!workspace)redirect('/login')

  const capacity=await getClientCapacity(workspace).catch(()=>null)
  ...
  try{
    ...
    const [t,c,s,a,aa,q,qd,qa,calc,finals,fd,fa]=await Promise.all([
      db.from('taxpayers').select('id',{count:'exact',head:true}).eq('firm_id',firmId),
      ...
      db.from('mtd_agents').select('id',{count:'exact',head:true}).eq('firm_id',firmId).eq('status','active'),
      db.from('mtd_agent_authorisations').select('id',{count:'exact',head:true}).eq('firm_id',firmId).eq('status','authorised'),
      db.from('hmrc_quarterly_submissions').select('id',{count:'exact',head:true}).eq('firm_id',firmId).eq('status','submitted'),
      db.from('hmrc_quarterly_submissions').select('id',{count:'exact',head:true}).eq('firm_id',firmId).eq('status','submitted').is('acting_agent_id',null),
      db.from('hmrc_quarterly_submissions').select('id',{count:'exact',head:true}).eq('firm_id',firmId).eq('status','submitted').not('acting_agent_id','is',null),
      db.from('mtd_submission_audit').select('id',{count:'exact',head:true}).eq('firm_id',firmId).eq('event_type','tax_calculation_retrieval').eq('status','accepted'),
  ...
  return <div className="shell"><aside className="side"><div className="brand"><img className="brandLogo" src="/mtd-lab-logo-post-login.svg" alt="MTD Lab"/></div><div className="nav"><Link href="/">Dashboard</Link><Link href="/taxpayers">Taxpayers</Link><Link href="/billing">Plans & Billing</Link><Link href="/agents">Agents</Link><span>Sandbox setup</span></div><div className="operator">Operated by Glomaxel IT Service</div></aside><main className="main">
    <div className="top"><div><h1 className="pageTitle">HMRC sandbox taxpayer setup</h1><p className="muted">Create and validate HMRC sandbox taxpayers for {workspace.firmName}. Metrics and reset actions on this page are restricted to this firm's workspace.</p></div><span className="badge">Sandbox only</span></div>
    {qs.error?<div className="status statusError" style={{marginTop:16}}><strong>Taxpayer workspace was not created.</strong><div>{qs.error}</div></div>:null}
    {atLimit?<div className="status statusError" style={{marginTop:16}}><strong>Client limit reached.</strong><div>You are using all {capacity?.capacity||0} client spaces. Add capacity before creating another taxpayer workspace. <Link href="/billing">Open Plans & Billing</Link>.</div></div>:null}
    ...
    <div className="status">The counters above show recorded Stage 2 evidence for this accounting firm only. Production submissions remain separately locked and must not be enabled simply because sandbox counters are nonzero.</div>
  </main></div>
}
