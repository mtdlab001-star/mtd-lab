...
export default async function ReadinessPage({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const eligibleDetail=eligible.length?focused?'The selected HMRC obligation is open and eligible for submission.':`${eligible.length} open cumulative obligation${eligible.length===1?'':'s'} from 2025/26 onward.`:focused?'The selected income source and period do not have a matching open HMRC obligation. Refresh HMRC data before submission.':'No open obligation starting on or after 6 April 2025 is currently stored.'
