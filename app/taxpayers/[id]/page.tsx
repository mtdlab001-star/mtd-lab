...
function annualStatus(taxYear?: string | null) {
  if (!taxYear) return 'Not Due Yet'
  return taxYearHasEnded(taxYear) ? 'Due' : 'Not Due Yet'
}

function businessDisplayName(business:any) {
  if (business?.business_name) return business.business_name
  const type = String(business?.business_type || '').toLowerCase()
  if (type.includes('foreign')) return 'Foreign Property'
  if (type.includes('property')) return 'UK Property'
...
function displayStatus(obligation: any, submission?: any) {
  ...
  if (hmrc === 'open' && quarterlyPeriodIsAvailable(String(obligation.period_end || ''))) return { label: 'Due', cls: 'statusOpen' }
...
export default async function TaxpayerPage({ params, searchParams }: { params: Promise<{id:string}>, searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const outstanding = eligibleOpen.filter(o => !acceptedSubmissionFor(submissions, o))
  const dueObligations = outstanding.filter(o => quarterlyPeriodIsAvailable(String(o.period_end || '')))
  const notDueYetObligations = outstanding.filter(o => !quarterlyPeriodIsAvailable(String(o.period_end || '')))
  const fulfilledObligations = eligibleObligations.filter(o => String(o.status || '').toLowerCase() === 'fulfilled')
  ...
  const obligationGroups = Array.from(groupedObligations.entries()).map(([businessId, rows]) => {
    ...
    const due = open.filter((o: any) => !acceptedSubmissionFor(submissions, o) && quarterlyPeriodIsAvailable(String(o.period_end || '')))
    const notDueYet = open.filter((o: any) => !acceptedSubmissionFor(submissions, o) && !quarterlyPeriodIsAvailable(String(o.period_end || '')))
    const groupNextDue = due.map((o: any) => o.due_date).filter(Boolean).sort()[0] || null
    return { businessId, label, businessName, sourceType, rows, total: rows.length, due: due.length, notDueYet: notDueYet.length, fulfilled: fulfilled.length, nextDue: groupNextDue }
  }).sort((a: any, b: any) => {
  ...
  const nextDue = dueObligations.map(o => o.due_date).filter(Boolean).sort()[0]
  const annualYear = selectedYear === 'all' ? eligibleYears[0] : selectedYear
  const annualReady = Boolean(annualYear && taxYearHasEnded(annualYear))
  ...
  return <div className="shell"><TaxpayerSidebar taxpayerId={id} active="overview" sourceTypes={sourceTypes}/><main className="main">
    ...
    <div className="cards"><div className="card"><span className="eyebrow">Connection</span><strong>{connectionStatus.usable?'Usable':'Action needed'}</strong><span className="muted">{connectionStatus.expired?'Refresh required':connectionStatus.connected?'HMRC authorisation':'Not connected'}</span></div><div className="card"><span className="eyebrow">Businesses</span><strong>{businesses.length}</strong><span className="muted">Income sources</span></div><div className="card"><span className="eyebrow">Due</span><strong>{dueObligations.length}</strong><span className="muted">{notDueYetObligations.length} Not Due Yet · {fulfilledObligations.length} Fulfilled</span></div><div className="card"><span className="eyebrow">Accepted pending HMRC</span><strong>{acceptedPending.length}</strong><span className="muted">Awaiting obligation refresh</span></div><div className="card"><span className="eyebrow">Next due</span><strong className="dateValue">{nextDue?fmtDate(nextDue):'None currently due'}</strong><span className="muted">2025/26 onward only</span></div></div>
    ...
    {acceptedPending.length>0&&<div className="status">{acceptedPending.length} quarterly update{acceptedPending.length===1?' has':'s have'} been accepted by HMRC while the obligation remains open. Synchronise HMRC to refresh the obligation status. These submissions are not counted as due again.</div>}
