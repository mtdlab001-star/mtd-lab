...
export default async function SubmissionCentre({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <div className="shell"><TaxpayerSidebar taxpayerId={id} active="submissions"/><main className="main">
  ...
  <div className="cards">{(['self-employment','uk-property','foreign-property'] as MtdIncomeSourceType[]).map(laneType=>{const lane=laneMeta[laneType];const count=sources.filter(source=>source.sourceType===laneType).length;return <div className="card" key={laneType}><span className="eyebrow">{lane.eyebrow}</span><strong className="dateValue">{lane.label}</strong><p className="muted">{lane.description}</p>{count?<Link className="btn btnSmall" href={`/taxpayers/${id}/submissions?type=${laneType}`}>{lane.action}</Link>:<span className="muted">No HMRC income source available</span>}</div>})}</div>
  ...
  <section className="panel"><div className="sectionHead"><div><h2>{meta.label} Submission</h2><p className="muted">Figures are submitted cumulatively from the beginning of the tax year to the selected period end.</p></div><a className="btn btnSmall" href={`/api/templates/${type}`}>Download Excel template</a></div>
