...
const foreignPropertyFields=[
 ['rents','Total Rents and Other Receipts'],['leasePremiums','Premiums of Lease Grant'],['otherIncome','Other Foreign Property Income'],['propertyExpenses','Allowable Property Expenses']
]
...
export default async function SubmissionCentre({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const {data:savedDraft}=selectedPeriod&&selectedBusiness?.business_id?await db.from('hmrc_quarterly_drafts').select('figures,updated_at').eq('taxpayer_id',id).eq('business_id',selectedBusiness.business_id).eq('income_source_type',type).eq('period_end',selectedPeriod.period_end).maybeSingle():({data:null} as any)
 const values=qs.imported?imported:(savedDraft?.figures||{})
 ...
 return <div className="shell"><TaxpayerSidebar taxpayerId={id} active="submissions" sourceTypes={availableTypes}/><main className="main">
  ...
  <section className="panel"><div className="sectionHead"><div><h2>{meta.label} Submission</h2><p className="muted">{savedDraft&&!qs.imported?'A saved draft has been restored for the selected cumulative period.':'Figures are submitted cumulatively from the beginning of the tax year to the selected period end.'}</p></div><a className="btn btnSmall" href={`/api/templates/${type}`}>Download Excel template</a></div>
   ...
   <form method="post" action="/api/hmrc/quarterly/prepare">
    ...
    <div className="detailGrid"><div><span className="eyebrow">Income source</span><strong>{selectedBusiness?.business_name||meta.label}</strong><div className="mono muted">{selectedBusiness?.business_id||'No matching HMRC income source'}</div>{selectedBusiness?.fallback&&<div className="muted">Identified from HMRC obligations</div>}</div><div><span className="eyebrow">Selected cumulative period</span><strong>{selectedPeriod?`${fmtDate(selectedPeriod.period_start)} to ${fmtDate(selectedPeriod.period_end)}`:'No open eligible period'}</strong><div className="muted">{selectedPeriod?.due_date?`Due ${fmtDate(selectedPeriod.due_date)}`:''}</div></div></div>
