...
export default async function CalculationsPage({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const [{data:taxpayer},{data:obligations},{data:submissions},{data:agentRows}]=await Promise.all([
  ...
  const {data:storedRows}=await db.from('mtd_submission_audit').select('calculation_id,response_summary,response_payload,hmrc_correlation_id,created_at').eq('taxpayer_id',id).eq('tax_year',selected).eq('event_type','tax_calculation_retrieval').eq('status','accepted').order('created_at',{ascending:false}).limit(1)
