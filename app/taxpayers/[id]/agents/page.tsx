    ...
    <section className="panel" style={{marginTop:20,marginBottom:16}}><h2>Add or authorise an agent</h2><p className="muted">HMRC authorisation and MTD Lab permissions are separate controls. Recording an agent here does not itself create HMRC authority. Use the HMRC Agent Services Account relationship where required, then record the relevant reference here.</p>
      <form method="post" action="/api/agents/authorise">
        ...
        <div className="detailGrid">
          <label><input type="checkbox" name="canViewRecords" defaultChecked/> View digital records</label>
          <label><input type="checkbox" name="canManageRecords"/> Manage digital records</label>
          <label><input type="checkbox" name="canViewObligations" defaultChecked/> View HMRC obligations</label>
          <label><input type="checkbox" name="canSubmitQuarterly"/> Submit quarterly updates</label>
          <label><input type="checkbox" name="canManageYearEnd"/> Manage year end</label>
          <label><input type="checkbox" name="canSubmitFinalDeclaration"/> Submit Final Declaration</label>
        </div>
        <label>Notes</label><textarea className="field" name="notes" rows={3}/>
        <button className="btn" type="submit">Authorise agent</button>
      </form>
    </section>
    <section className="panel"><div className="sectionHead"><div><h2>Authorised agents</h2><p className="muted">Only active, unexpired authorisations should be used for delegated MTD actions.</p></div><strong>{rows.filter((r:any)=>r.status==='authorised').length} active</strong></div>
      <div className="tableWrap"><table><thead><tr><th>Agent</th><th>HMRC ARN</th><th>Status</th><th>Expires</th><th>Quarterly</th><th>Year end</th><th>Final Declaration</th><th>Action</th></tr></thead><tbody>{rows.length?rows.map((r:any)=><tr key={r.id}><td><strong>{r.mtd_agents?.agent_name||'Agent'}</strong><div className="muted">{r.mtd_agents?.organisation_name||''}</div></td><td className="mono">{r.mtd_agents?.hmrc_arn||'Not recorded'}</td><td><span className={`statusPill ${r.status==='authorised'?'statusDone':'statusOpen'}`}>{r.status}</span></td><td>{fmt(r.expires_at)}</td><td>{r.can_submit_quarterly?'Allowed':'No'}</td><td>{r.can_manage_year_end?'Allowed':'No'}</td><td>{r.can_submit_final_declaration?'Allowed':'No'}</td><td>{r.status==='authorised'?<form method="post" action="/api/agents/revoke"><input type="hidden" name="taxpayerId" value={id}/><input type="hidden" name="authorisationId" value={r.id}/><button className="btn btnSmall" type="submit">Revoke</button></form>:<span className="muted">No action</span>}</td></tr>):<tr><td colSpan={8} className="empty">No agent authorisations recorded for this taxpayer.</td></tr>}</tbody></table></div>
    </section>
  </main></div>
}
