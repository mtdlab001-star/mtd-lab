...
export default async function EndOfYearPage({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <div className="shell"><TaxpayerSidebar taxpayerId={id} active="end-of-year"/><main className="main">
  ...
  <section className="panel"><div className="sectionHead"><div><h2>Year end readiness</h2><p className="muted">MTD Lab uses HMRC data and the application's submission audit trail to show what is complete and what still needs attention.</p></div><form method="get"><select className="selectField" name="taxYear" defaultValue={selected}>{years.length?years.map(y=><option key={y} value={y}>{y}</option>):<option value={selected}>{selected}</option>}</select><button className="btn btnSmall" type="submit">View tax year</button></form></div>
   <div className="cards">
    <div className="card"><span className="eyebrow">Income sources</span><strong>{incomeSourcesReady?'Ready':'Needs attention'}</strong><p className="muted">{businessCount} HMRC income source{businessCount===1?'':'s'} found{hasProperty?', including UK property':''}.</p></div>
    <div className="card"><span className="eyebrow">Quarterly obligations</span><strong>{quarterlyReady?'Complete':`${open.length} open`}</strong><p className="muted">{yearObligations.length} modern obligation{yearObligations.length===1?'':'s'} retrieved for {selected}.</p></div>
    <div className="card"><span className="eyebrow">Accepted updates</span><strong>{accepted.length}</strong><p className="muted">Accepted cumulative quarterly submissions recorded by MTD Lab for this tax year.</p></div>
    <div className="card"><span className="eyebrow">Tax year status</span><strong>{ended?'Ended':'Still in progress'}</strong><p className="muted">Final Declaration remains guarded until the tax year has ended and HMRC calculation requirements are satisfied.</p></div>
   </div>
  </section>
  <section className="panel" style={{marginTop:16}}><h2>Completion checklist</h2><div className="tableWrap"><table><thead><tr><th>Step</th><th>Status</th><th>Action</th></tr></thead><tbody>
   ...
   <tr><td>Annual adjustments, losses and other Self Assessment information</td><td><span className="statusPill statusOpen">Review required</span></td><td><span className="muted">Dedicated HMRC schedules will be added here as each API is connected.</span></td></tr>
  ...
  <section className="panel" style={{marginTop:16}}><h2>Intent to finalise</h2><p className="muted">This asks HMRC to generate the calculation used for the end of year journey. It does not itself make the Final Declaration.</p>
   {!ended&&<div className="status statusError"><strong>Tax year still in progress.</strong><div className="muted">MTD Lab will not present this as a normal Final Declaration route before 5 April at the end of the selected tax year.</div></div>}
   <form method="post" action="/api/hmrc/calculations/trigger"><input type="hidden" name="taxpayerId" value={id}/><input type="hidden" name="taxYear" value={selected}/><input type="hidden" name="calculationType" value="intent-to-finalise"/><FraudContextFields/><button className="btn" type="submit" disabled={!ended||!incomeSourcesReady}>Request intent to finalise calculation</button></form>
   <div style={{marginTop:12}}><Link className="btn btnSmall" href={`/taxpayers/${id}/calculations?taxYear=${encodeURIComponent(selected)}`}>HMRC Tax Calculation</Link></div>
  </section>
 </main></div>
}
