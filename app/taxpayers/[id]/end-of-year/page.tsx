...
export default async function EndOfYearPage({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const [{data:reviews},{data:calculations}]=await Promise.all([
  db.from('mtd_year_end_reviews').select('section,status,note,reviewed_at').eq('taxpayer_id',id).eq('firm_id',workspace.firmId).eq('tax_year',selected),
  db.from('mtd_submission_audit').select('calculation_id,created_at').eq('taxpayer_id',id).eq('firm_id',workspace.firmId).eq('tax_year',selected).eq('event_type','tax_calculation_retrieval').eq('status','accepted').order('created_at',{ascending:false}).limit(1)
 ])
 ...
 const acceptedCalculation:any=calculations?.[0]||null
 const calculationReady=Boolean(acceptedCalculation?.calculation_id)
 ...
 return <div className="shell"><TaxpayerSidebar taxpayerId={id} active="end-of-year"/><main className="main">
  ...
  <section className="panel" style={{marginTop:16}}><h2>Completion checklist</h2><div className="tableWrap"><table><thead><tr><th>Step</th><th>Status</th><th>Open schedule</th><th>Review control</th></tr></thead><tbody>
   ...
   <tr><td>Quarterly obligations completed</td><td><span className={`statusPill ${quarterlyReady?'statusDone':'statusOpen'}`}>{quarterlyChecklist}</span></td><td><Link className="btn btnSmall" href={`/taxpayers/${id}/quarterly`}>Review obligations</Link></td><td>HMRC driven</td></tr>
   ...
   <tr><td>HMRC tax calculation</td><td><span className={`statusPill ${calculationReady?'statusDone':'statusOpen'}`}>{calculationReady?'Retrieved':'Required'}</span>{calculationReady&&<div className="muted" style={{marginTop:4}}>Calculation ID: <span className="mono">{acceptedCalculation.calculation_id}</span></div>}</td><td><Link className="btn btnSmall" href={`/taxpayers/${id}/calculations?taxYear=${encodeURIComponent(selected)}`}>Open calculations</Link></td><td>HMRC driven</td></tr>
  </tbody></table></div></section>
  <section className="panel" style={{marginTop:16}}><h2>Intent to finalise</h2><p className="muted">This asks HMRC to generate the calculation used for the end of year journey. It does not itself make the Final Declaration.</p>{!canFinalise&&<div className="status statusError"><strong>Intent to finalise is currently blocked.</strong><div className="muted">{blockers.join('. ')}.</div></div>}<form method="post" action="/api/hmrc/calculations/trigger"><input type="hidden" name="taxpayerId" value={id}/><input type="hidden" name="taxYear" value={selected}/><input type="hidden" name="calculationType" value="intent-to-finalise"/>{canFinalise&&<FraudContextFields/>}{canFinalise?<button className="btn" type="submit">Request intent to finalise calculation</button>:<button className="btn btnSmall" type="button" disabled aria-disabled="true" title="Finalisation becomes available after the tax year has ended and all required quarterly obligations are complete." style={{opacity:.55,cursor:'not-allowed'}}>Available after tax year end and quarterly completion</button>}</form><div style={{marginTop:12}}><Link className="btn btnSmall" href={`/taxpayers/${id}/calculations?taxYear=${encodeURIComponent(selected)}`}>HMRC Tax Calculation</Link></div></section>
 </main></div>
}
