...
export default async function BusinessesPage({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const active=view==='property'?'property':view==='self-employment'?'self-employment':'businesses'
 return <div className="shell"><TaxpayerSidebar taxpayerId={id} active={active}/><main className="main"><div className="top"><div><h1 className="pageTitle">{title}</h1><p className="muted">Manage business details and retrieve information from HMRC.</p></div><Link className="btn btnSmall" href={`/taxpayers/${id}`}>← Back</Link></div>
