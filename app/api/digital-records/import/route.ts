...
export async function POST(req:Request){
 ...
 try{sources=await listMtdIncomeSources(db,taxpayerId,workspace.firmId)}catch(e:any){back.searchParams.set('error',e.message||'Could not validate HMRC income sources.');return NextResponse.redirect(back,303)}
 ...
 for(let index=0;index<validation.inserts.length;index++){
  ...
  const {data:duplicate}=await db.from('mtd_digital_records').select('id').eq('firm_id',workspace.firmId).eq('taxpayer_id',taxpayerId).eq('business_id',candidate.business_id).eq('transaction_date',candidate.transaction_date).eq('record_type',candidate.record_type).eq('category',candidate.category).eq('amount',candidate.amount).limit(1)
  if(duplicate?.length)issues.push({row:validation.rowNumbers[index]||index+2,message:'duplicate of an existing digital record'});else inserts.push({...candidate,firm_id:workspace.firmId})
 }
