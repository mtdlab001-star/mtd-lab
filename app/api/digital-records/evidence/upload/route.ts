...
export async function POST(req:Request){
 ...
 if(!await taxpayerBelongsToWorkspace(taxpayerId,workspace)){back.searchParams.set('error','Digital record was not found.');return NextResponse.redirect(back,303)}
 ...
 const db=supabaseAdmin();const {data:row}=await db.from('mtd_digital_records').select('id,taxpayer_id,business_id,tax_year,document_url').eq('firm_id',workspace.firmId).eq('id',recordId).eq('taxpayer_id',taxpayerId).maybeSingle()
 if(!row){back.searchParams.set('error','Digital record was not found.');return NextResponse.redirect(back,303)}
 const objectPath=`${workspace.firmId}/${taxpayerId}/${row.business_id}/${recordId}/${crypto.randomUUID()}-${safeName(file.name)}`
 ...
 const {error:updateError}=await db.from('mtd_digital_records').update({document_url:objectPath,document_name:file.name,document_mime_type:file.type,document_size:file.size,document_uploaded_at:new Date().toISOString()}).eq('firm_id',workspace.firmId).eq('id',recordId).eq('taxpayer_id',taxpayerId)
