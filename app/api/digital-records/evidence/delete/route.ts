...
export async function POST(req:Request){
 ...
 if(!taxpayerId||!recordId||!await taxpayerBelongsToWorkspace(taxpayerId,workspace)){back.searchParams.set('error','Supporting document was not found.');return NextResponse.redirect(back,303)}
 const db=supabaseAdmin();const {data:row}=await db.from('mtd_digital_records').select('id,business_id,tax_year,document_url,document_name').eq('firm_id',workspace.firmId).eq('id',recordId).eq('taxpayer_id',taxpayerId).maybeSingle()
 ...
 const {error:updateError}=await db.from('mtd_digital_records').update({document_url:null,document_name:null,document_mime_type:null,document_size:null,document_uploaded_at:null}).eq('firm_id',workspace.firmId).eq('id',recordId).eq('taxpayer_id',taxpayerId)
 if(updateError){back.searchParams.set('error',updateError.message);return NextResponse.redirect(back,303)}
