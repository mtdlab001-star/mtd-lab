...
export async function GET(req:Request){
 ...
 const rows=[
  ...quarterlyRows.map((r:any)=>({date:r.submitted_at||r.created_at,type:'Quarterly update',actor:actor(r),status:reconciliationStatus(r,obs),reference:r.hmrc_correlation_id||'',detail:`${sourceType(r)} · Period end ${r.period_end||'Not available'}${r.hmrc_http_status?` · HMRC HTTP ${r.hmrc_http_status}`:''}`})),
