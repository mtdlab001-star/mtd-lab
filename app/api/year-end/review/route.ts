...
export async function POST(req:Request){
 ...
 const db=supabaseAdmin();const {data:taxpayer}=await db.from('taxpayers').select('id').eq('id',taxpayerId).eq('firm_id',workspace.firmId).maybeSingle();if(!taxpayer){back.searchParams.set('reviewError','Taxpayer is not available in this accounting workspace');return NextResponse.redirect(back,303)}
 const now=new Date().toISOString();const {error}=await db.from('mtd_year_end_reviews').upsert({firm_id:workspace.firmId,taxpayer_id:taxpayerId,tax_year:taxYear,section,status,note:note||null,reviewed_at:now,updated_at:now},{onConflict:'taxpayer_id,tax_year,section'})
