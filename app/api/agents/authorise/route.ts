...
export async function POST(req:Request){
  ...
  const existingAgentId=String(form.get('existingAgentId')||'').trim()
  ...
  if(!taxpayerId||(!existingAgentId&&!agentName)){back.searchParams.set('error','Choose an existing agent or enter a new agent name.');return NextResponse.redirect(back,303)}
  ...
  try{
    ...
    let agent:any=null
    if(existingAgentId){const {data,error}=await db.from('mtd_agents').select('*').eq('id',existingAgentId).eq('firm_id',workspace.firmId).eq('status','active').maybeSingle();if(error)throw error;if(!data){back.searchParams.set('error','The selected firm agent is not available.');return NextResponse.redirect(back,303)}agent=data}
    if(!agent&&hmrcArn){const {data}=await db.from('mtd_agents').select('*').eq('firm_id',workspace.firmId).eq('hmrc_arn',hmrcArn).maybeSingle();agent=data}
    ...
    if(!agent){const {data,error}=await db.from('mtd_agents').insert({firm_id:workspace.firmId,agent_name:agentName,organisation_name:organisationName,hmrc_arn:hmrcArn,email,status:'active',updated_at:new Date().toISOString()}).select('*').single();if(error)throw error;agent=data}else if(!existingAgentId){const {error}=await db.from('mtd_agents').update({agent_name:agentName,organisation_name:organisationName,hmrc_arn:hmrcArn||agent.hmrc_arn,email:email||agent.email,status:'active',updated_at:new Date().toISOString()}).eq('id',agent.id).eq('firm_id',workspace.firmId);if(error)throw error}
