...
export async function POST(req:Request){
  ...
  try{
    const {data:taxpayer,error:taxpayerError}=await db.from('taxpayers').select('id').eq('id',taxpayerId).maybeSingle();if(taxpayerError)throw taxpayerError;if(!taxpayer){back.searchParams.set('error','Taxpayer workspace was not found.');return NextResponse.redirect(back,303)}
    let agent:any=null
