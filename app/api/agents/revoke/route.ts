...
export async function POST(req:Request){
  ...
  try{
    const db=supabaseAdmin();const {data,error}=await db.from('mtd_agent_authorisations').update({status:'revoked',revoked_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',authorisationId).eq('firm_id',workspace.firmId).eq('taxpayer_id',taxpayerId).select('id').maybeSingle()
