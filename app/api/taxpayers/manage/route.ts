...
import { assertClientCapacity } from '@/lib/subscription-capacity'
...
export async function POST(req: Request) {
 ...
 try {
  ...
  const { data: taxpayer, error: lookupError } = await db.from('taxpayers').select('id,display_name,archived_at').eq('id', taxpayerId).eq('firm_id',workspace.firmId).maybeSingle()
  if (lookupError) throw lookupError
  if (!taxpayer) return NextResponse.json({ error: 'Client not found' }, { status: 404 })
  if (action === 'archive' || action === 'restore') {
   if(action==='restore'&&taxpayer.archived_at){try{await assertClientCapacity(workspace)}catch(cause){return NextResponse.json({error:cause instanceof Error?cause.message:'Client capacity could not be verified'},{status:409})}}
   const archivedAt = action === 'archive' ? new Date().toISOString() : null
   const { data, error } = await db.from('taxpayers').update({ archived_at: archivedAt, updated_at: new Date().toISOString() }).eq('id', taxpayerId).eq('firm_id',workspace.firmId).select('id').maybeSingle()
   if (error) throw error
   if (!data) return NextResponse.json({ error: 'Client was not updated' }, { status: 409 })
   return NextResponse.json({ success: true, action })
  }
  if (confirmation !== taxpayer.display_name) return NextResponse.json({ error: 'Enter the client name exactly to confirm permanent deletion' }, { status: 400 })
  const { data: deleted, error: deleteError } = await db.from('taxpayers').delete().eq('id', taxpayerId).eq('firm_id',workspace.firmId).select('id').maybeSingle()
