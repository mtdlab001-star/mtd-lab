...
export async function POST(req:Request){
 ...
 try{await assertClientCapacity(workspace)}catch(cause){const message=cause instanceof Error?cause.message:'Client capacity could not be verified';return NextResponse.redirect(new URL(`/taxpayers/sandbox?error=${encodeURIComponent(message)}`,req.url),303)}
