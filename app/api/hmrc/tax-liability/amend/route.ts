...
import { assertSandboxFeature, compatibilityAmount } from '@/lib/hmrc-compatibility'
...
export async function POST(req:Request){
 ...
 try{refunded=compatibilityAmount(form.get('taxRefundedOrSetOff'),'Tax refunded or set off');assertSandboxFeature('Tax refunded or set off',refunded!==undefined);if(refunded!==undefined&&Number(taxYear.slice(0,4))<2026)throw new Error('Tax refunded or set off requires tax year 2026 to 2027 onwards.')}catch(e:any){back.searchParams.set('error',e.message);return NextResponse.redirect(back,303)}
 const payload=clean({carryBackLossesDecrease:{incomeTax:amount(form,'incomeTax'),class4:amount(form,'class4'),capitalGainsTax:amount(form,'capitalGainsTax')},taxRefundedOrSetOff:{amount:refunded}})
