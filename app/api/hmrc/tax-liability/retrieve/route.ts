...
async function handle(req:Request,source:URLSearchParams|FormData){
 ...
 try{const res=await fetch(`${hmrcApiBase}/individuals/tax-liability/adjustments/${encodeURIComponent(taxpayer.nino)}/${encodeURIComponent(taxYear)}`,{headers:{Authorization:`Bearer ${token}`,Accept:'application/vnd.hmrc.1.0+json',...(process.env.HMRC_ENVIRONMENT==='production'?{}:{'Gov-Test-Scenario':'DEFAULT'})},cache:'no-store'});const text=await res.text();let data:any={};try{data=text?JSON.parse(text):{}}catch{data={raw:text}}const correlationId=res.headers.get('x-correlationid')||res.headers.get('x-correlation-id')||'';if(!res.ok){back.searchParams.set('error',res.status===404?'HMRC has no tax liability adjustments for this tax year.':(data?.message||data?.code||`HMRC ${res.status}`));if(correlationId)back.searchParams.set('correlationId',correlationId);return NextResponse.redirect(back,303)}back.searchParams.set('retrieved','1');back.searchParams.set('result',Buffer.from(JSON.stringify(data)).toString('base64url'));if(correlationId)back.searchParams.set('correlationId',correlationId);return NextResponse.redirect(back,303)}catch(e:any){back.searchParams.set('error',e.message||'Could not retrieve HMRC tax liability adjustments');return NextResponse.redirect(back,303)}
}
