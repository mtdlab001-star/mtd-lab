...
export async function POST(req:Request){
  ...
  const [{data:taxpayer},{data:conn}]=await Promise.all([
    db.from('taxpayers').select('nino').eq('id',taxpayerId).maybeSingle(),
    db.from('hmrc_connections').select('access_token').eq('taxpayer_id',taxpayerId).maybeSingle()
  ])
  if(!taxpayer?.nino||!conn?.access_token){back.searchParams.set('error','HMRC taxpayer connection is incomplete');return NextResponse.redirect(back,303)}

  const fraud=buildFraudHeaders(req,form,taxpayerId)
  if(fraud.missing.length){
    back.searchParams.set('readiness','blocked')
    back.searchParams.set('missing',fraud.missing.join(','))
  ...
  try{
    const res=await fetch(`${hmrcApiBase}${endpoint}`,{
      ...
      body:JSON.stringify(requestPayload),cache:'no-store'
    })
