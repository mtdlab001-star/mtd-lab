...
export async function POST(req:Request){
 ...
 const back=new URL(`/taxpayers/${encodeURIComponent(taxpayerId)}/end-of-year/reliefs`,req.url);back.searchParams.set('taxYear',taxYear);back.searchParams.set('type','charitable')
 ...
 const payload=clean({giftAidPayments:{totalAmount:num(form,'totalAmount'),oneOffAmount:num(form,'oneOffAmount'),amountTreatedAsPreviousTaxYear:num(form,'amountTreatedAsPreviousTaxYear'),amountTreatedAsSpecifiedTaxYear:num(form,'amountTreatedAsSpecifiedTaxYear')},gifts:{landAndBuildings:num(form,'landAndBuildings'),sharesOrSecurities:num(form,'sharesOrSecurities')}})
 if(payload.giftAidPayments&&Object.keys(payload.giftAidPayments).length===0)delete payload.giftAidPayments;if(payload.gifts&&Object.keys(payload.gifts).length===0)delete payload.gifts
 if(Object.keys(payload).length===0){back.searchParams.set('error','Enter at least one charitable giving relief amount');return NextResponse.redirect(back,303)}
 ...
 try{const res=await fetch(`${hmrcApiBase}/individuals/reliefs/charitable-giving/${encodeURIComponent(taxpayer.nino)}/${encodeURIComponent(taxYear)}`,{method:'PUT',headers:{Authorization:`Bearer ${token}`,Accept:'application/vnd.hmrc.3.0+json','Content-Type':'application/json',...(process.env.HMRC_ENVIRONMENT==='production'?{}:{'Gov-Test-Scenario':'DEFAULT'}),...fraud.headers},body:JSON.stringify(payload),cache:'no-store'});const text=await res.text();let data:any={};try{data=text?JSON.parse(text):{}}catch{data={raw:text}}const correlationId=res.headers.get('x-correlationid')||res.headers.get('x-correlation-id')||'';const auditId=await recordHmrcResponse(db,{taxpayerId,taxYear,eventType:'reliefs_charitable_amendment',status:res.ok?'accepted':'rejected',payload:data,correlationId,hmrcStatus:res.status,requestSummary:{type:'charitable'}});if(auditId)back.searchParams.set('auditId',auditId);if(!res.ok){back.searchParams.set('error',data?.message||data?.code||`HMRC ${res.status}`);if(correlationId)back.searchParams.set('correlationId',correlationId);return NextResponse.redirect(back,303)}await markYearEndReviewed(taxpayerId,taxYear,'reliefs','Charitable Giving relief updated in HMRC');back.searchParams.set('saved','1');if(correlationId)back.searchParams.set('correlationId',correlationId);return NextResponse.redirect(back,303)}catch(e:any){back.searchParams.set('error',e.message||'Could not submit charitable giving relief');return NextResponse.redirect(back,303)}
}
