...
export async function POST(req:Request){
 ...
 try{if(!/^20\d{2}-\d{2}$/.test(taxYear))throw new Error('Select a valid HMRC tax year');if(!taxYearHasEnded(taxYear))throw new Error('Relief figures can be prepared now, but HMRC submission is locked until the selected tax year has ended.');if(process.env.HMRC_ENVIRONMENT==='production'&&process.env.HMRC_ALLOW_PRODUCTION_SUBMISSIONS!=='true')throw new Error('Production HMRC submissions are locked until explicitly enabled.');const vct:any=item(f,'vct');const eis:any=item(f,'eis');const community:any=item(f,'community');const seed:any=item(f,'seed','companyName');if(eis)eis.knowledgeIntensive=String(f.get('eisKnowledgeIntensive')||'false')==='true';const payload:any={};if(vct)payload.vctSubscription=[vct];if(eis)payload.eisSubscription=[eis];if(community)payload.communityInvestment=[community];if(seed)payload.seedEnterpriseInvestment=[seed];if(!Object.keys(payload).length)throw new Error('Enter at least one investment relief claim')
 const {data:taxpayer}=await supabaseAdmin().from('taxpayers').select('nino').eq('id',taxpayerId).eq('firm_id',workspace.firmId).maybeSingle();if(!taxpayer?.nino)throw new Error('Taxpayer is not available in this accounting workspace or has no NINO');const token=await getValidHmrcAccessToken(taxpayerId);const fraud=buildFraudHeaders(req,f,taxpayerId);if(fraud.missing.length)throw new Error(`Missing HMRC fraud prevention data: ${fraud.missing.join(', ')}`)
 const res=await fetch(`${hmrcApiBase}/individuals/reliefs/investment/${encodeURIComponent(taxpayer.nino)}/${encodeURIComponent(taxYear)}`,{method:'PUT',headers:{Authorization:`Bearer ${token}`,Accept:hmrcAcceptHeader('reliefs'),'Content-Type':'application/json',...(process.env.HMRC_ENVIRONMENT==='production'?{}:{'Gov-Test-Scenario':'DEFAULT'}),...fraud.headers},body:JSON.stringify(payload),cache:'no-store'});const raw=await res.text();let data:any={};try{data=raw?JSON.parse(raw):{}}catch{data={raw}}const correlationId=res.headers.get('x-correlationid')||res.headers.get('x-correlation-id')||'';if(!res.ok){back.searchParams.set('error',data?.message||data?.code||`HMRC ${res.status}`);if(correlationId)back.searchParams.set('correlationId',correlationId);return NextResponse.redirect(back,303)}await markYearEndReviewed(taxpayerId,taxYear,'reliefs','Investment reliefs updated in HMRC');back.searchParams.set('savedInvestment','1');if(correlationId)back.searchParams.set('correlationId',correlationId);return NextResponse.redirect(back,303)
