...
export async function GET(req:Request){
 const url=new URL(req.url);const taxpayerId=String(url.searchParams.get('taxpayerId')||'demo');const taxYear=String(url.searchParams.get('taxYear')||'');const type=String(url.searchParams.get('type')||'other')
 ...
 try{const res=await fetch(`${hmrcApiBase}${endpoint}`,{headers:{Authorization:`Bearer ${token}`,Accept:'application/vnd.hmrc.3.0+json',...(process.env.HMRC_ENVIRONMENT==='production'?{}:{'Gov-Test-Scenario':'DEFAULT'})},cache:'no-store'});const text=await res.text();let payload:any={};try{payload=text?JSON.parse(text):{}}catch{payload={raw:text}}const correlationId=res.headers.get('x-correlationid')||res.headers.get('x-correlation-id')||'';if(!res.ok){back.searchParams.set('error',res.status===404?'No HMRC relief data was found for this category.':(payload?.message||payload?.code||`HMRC ${res.status}`));if(correlationId)back.searchParams.set('correlationId',correlationId);return NextResponse.redirect(back,303)}await markYearEndReviewed(taxpayerId,taxYear,'reliefs',`Retrieved ${type} reliefs from HMRC`);back.searchParams.set('retrieved','1');back.searchParams.set('result',Buffer.from(JSON.stringify(payload)).toString('base64url'));if(correlationId)back.searchParams.set('correlationId',correlationId);return NextResponse.redirect(back,303)}catch(e:any){back.searchParams.set('error',e.message||'Could not retrieve HMRC reliefs');return NextResponse.redirect(back,303)}
}
