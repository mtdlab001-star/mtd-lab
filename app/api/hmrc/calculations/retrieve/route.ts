...
export async function GET(req:Request){
 ...
 try{
  ...
  await db.from('mtd_submission_audit').insert({taxpayer_id:taxpayerId,tax_year:taxYear,event_type:'tax_calculation_retrieval',status:res.ok?'accepted':'rejected',calculation_id:calculationId,hmrc_status:res.status,correlation_id:correlationId||null,response_payload:payload,submitted_at:new Date().toISOString()}).catch(()=>null)
