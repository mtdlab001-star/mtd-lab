...
export async function POST(req:Request){
 ...
 const form=await req.formData();const taxpayerId=String(form.get('taxpayerId')||'demo');const taxYear=String(form.get('taxYear')||'');const calculationId=String(form.get('calculationId')||'');const actingAgentId=String(form.get('actingAgentId')||'').trim()||null
 ...
 try{
  const res=await fetch(`${hmrcApiBase}${endpoint}`,{headers:{Authorization:`Bearer ${token}`,Accept:hmrcAcceptHeader('individualCalculations'),...fraud.headers},cache:'no-store'})
