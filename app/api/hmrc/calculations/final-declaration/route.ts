...
export async function POST(req:Request){
 ...
 const db=supabaseAdmin();const [{data:taxpayer},{count:businessCount},{data:obs},{data:reviews},{data:calcs}]=await Promise.all([db.from('taxpayers').select('nino').eq('id',taxpayerId).eq('firm_id',workspace.firmId).maybeSingle(),db.from('hmrc_businesses').select('id',{count:'exact',head:true}).eq('firm_id',workspace.firmId).eq('taxpayer_id',taxpayerId),db.from('hmrc_obligations').select('period_start,period_end,status').eq('firm_id',workspace.firmId).eq('taxpayer_id',taxpayerId).gte('period_start',period.from).lte('period_end',period.to),db.from('mtd_year_end_reviews').select('section,status').eq('firm_id',workspace.firmId).eq('taxpayer_id',taxpayerId).eq('tax_year',taxYear),db.from('mtd_submission_audit').select('id').eq('firm_id',workspace.firmId).eq('taxpayer_id',taxpayerId).eq('tax_year',taxYear).eq('event_type','tax_calculation_retrieval').eq('status','accepted').eq('calculation_id',calculationId).limit(1)])
 if(!taxpayer?.nino){back.searchParams.set('error','Taxpayer is not available in this accounting workspace or has no NINO');return NextResponse.redirect(back,303)}
